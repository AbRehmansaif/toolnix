...
export default function JpgToPdf() {
  ...
  const addFiles = (incoming) => {
    const valid = Array.from(incoming).filter(f => f.type === 'image/jpeg' || f.type === 'image/jpg');
    setFiles(prev => {
  ...
  return (
    <div className="tool-page">
      <div className="tool-breadcrumb">
        ...
        <ChevronRight size={14} className="tool-breadcrumb-sep" />
        <span className="tool-breadcrumb-current">JPG to PDF</span>
      </div>

      <div className="tool-header">
        <div className="tool-header-inner">
          <div className="tool-header-icon" style={{ background: '#fff7ed' }}>
            <FileImage size={36} color="#f97316" strokeWidth={1.6} />
          </div>
          <div className="tool-header-content">
            <div className="tool-header-title">JPG to PDF</div>
      ...
      <div className="tool-main">
        <div>
          ...
          >
            <input ref={inputRef} type="file" accept="image/jpeg,image/jpg" multiple style={{ display: 'none' }} onChange={e => addFiles(e.target.files)} />
            <div className="upload-zone-icon"><Upload size={32} color="#f97316" /></div>
            <div className="upload-zone-title">Drop JPG images here</div>
            <div className="upload-zone-btn" style={{ background: 'linear-gradient(135deg,#f97316,#ea580c)' }}>
              <Upload size={14} /> Select JPG images
            </div>
          </div>
            ...
            <div className="file-list">
              {files.map((f, i) => (
                <div key={i} className="file-item">
                  <div className="file-item-icon" style={{ background: '#fff7ed' }}><ImageIcon size={18} color="#f97316" /></div>
